...
- 候选 ID 和设计意图；
- 物料 ID、当前批次或寻源规格、比例；比例以百分比表示、最多保留两位小数，显示值合计必须落在 `99.95%–100.05%`；
...
1. 设定历史基线或对照样；
2. 只选择能够回答决策问题的候选和烘焙变量；一个“样品”指一次独立配方与烘焙组合形成的物理熟豆样，历史基线或新做对照也各算一个样品，总数不超过输入上限；
3. 固定并记录批次、投料、设备、曲线或关键参数、养豆时间、研磨、水、温度、冲煮和奶咖条件；
4. 样品盲码，评价顺序随机；按输入协议由多名有辨识能力的人独立记录；
5. 同时记录描述性结果和偏好或适配判断，不把两者混在一起；
6. 每个物理熟豆样先后在黑咖标准条件与客户真实饮用场景下评价；这是同一样品的两组评价记录，不重复计算为两个样品；
...
| `G2_FORMULA` | 输出候选前 | 组分可识别、比例为正且最多两位小数，显示值合计在 `99.95%–100.05%`，符合硬卖点或禁忌 | 违反 100% 阿拉比卡等硬约束，或比例超出容差 | 规格未定时只能进入寻源或补数分支 |
...
- `physical_sample_count`：独立的“配方版本 × 烘焙方向”物理熟豆样数量，包含本轮新做的基线或对照；它受 `max_first_round_samples` 约束；
- `evaluation_record_count`：物理样品在不同评价场景下产生的记录数。本 Case 每个物理样都有黑咖和奶咖两组记录，因此为 `physical_sample_count × 2`，但不占用额外样品名额。

同一配方若采用两个烘焙方向，必须算两个物理样；把同一熟豆样分别冲成黑咖和奶咖，仍只算一个物理样。
